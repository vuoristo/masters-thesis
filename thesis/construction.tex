...
paths in the network are independent. \fixme{does this need more reasoning?}
...
both of the processing paths are dummy actors \fixme{should these dummy actors
be killed? it's not negilible amount of work, because the preesm
measurements should be redone...}. The pentagons in the top and the
...
on the Y channel. \fixme{explain YUV format and how it's used here including the
omission of U and V}
...
frames, corresponding to the filter kernel sizes.\fixme{why?} With the black
...
multi-leveled loop structure would suggest \fixme{ref to comp manual}. The
...
starts the data export. \fixme{move up where there is talk about dummy actors?}
